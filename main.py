...
flags.DEFINE_bool("continuation", False, "Whether to continue training saved model.")
flags.DEFINE_float("learning_rate", 1e-4, "Learning rate for training.")
flags.DEFINE_float("discount", 0.99, "Discount rate for future rewards.")
flags.DEFINE_integer("max_steps", int(3000), "Total episodes trained for.")
flags.DEFINE_integer("snapshot_step", int(2e2), "How many steps before saving model.")
...
flags.DEFINE_enum("agent_race", "random", sc2_env.Race.__members__.keys(), "Agent's race.")
flags.DEFINE_enum("bot_race", "random", sc2_env.Race.__members__.keys(), "Bot's race. ")
flags.DEFINE_enum("difficulty", None, sc2_env.Difficulty.__members__.keys(), "Bot's strength. Must specify if you want a bot")
flags.DEFINE_integer("max_agent_steps", 840, "Max agent steps per episode.")
...
def run_thread(agent, map_name, visualize):
  ...
  with sc2_env.SC2Env(
    ...
    agent_interface_format=features.AgentInterfaceFormat(
      feature_dimensions=features.Dimensions(screen=(FLAGS.screen_resolution,FLAGS.screen_resolution),
                          minimap=(FLAGS.minimap_resolution,FLAGS.minimap_resolution)),
      rgb_dimensions=features.Dimensions(screen=(FLAGS.screen_resolution,FLAGS.screen_resolution),
                          minimap=(FLAGS.minimap_resolution,FLAGS.minimap_resolution)),
